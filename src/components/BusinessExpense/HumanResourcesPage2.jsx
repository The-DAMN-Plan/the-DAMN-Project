...
export default function HumanResourcesPage1() {
    ...
    const handleSubmit = (event) => {
        ...
        const userInput = [{ expense_name: expenseName, service, frequency, expense_amount: expenseAmount, type: 'business hr', year: 2, budget_id: Number(budgetId.budgetId) }]
    ...
    const filteredExpenses = expenses.filter(item => item.type === 'business hr' && item.year === 2);


    return (
        <Main open={open}>
            <Container >
                <Typography variant="h3" color={'primary'} textAlign={'center'} gutterBottom>Human Resource Budget Year 2</Typography>
                ...
                <ProgressBar back={'hrpagey1'} next={'otherbusiness'} value={84} budgetId={budgetId} />
            </Container>
            <Footer />
        </Main>
